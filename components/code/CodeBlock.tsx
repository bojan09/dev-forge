...
function tokenize(code: string, lang: string): string {
  ...
  const C = {
    ...
    comment:  (s: string) => `<span style="color:#6e7681;font-style:italic">${s}</span>`,
  ...
  if (isBash) {
    result = result
      ...
      .replace(/\b(export|source|\.|cd|ls|cat|echo|npm|npx|git|mkdir|rm|cp|mv|sudo|chmod|grep|find|sed|awk|curl|wget)\b/g, (m) => C.keyword(m))
  ...
  result = result
    ...
    .replace(/(\/\/.*$)/gm,                      (m) => C.comment(m))
    .replace(/(\/\*[\s\S]*?\*\/)/g,              (m) => C.comment(m))
    .replace(/(#.*$)/gm, lang === "python" || lang === "yaml" ? (m) => C.comment(m) : "$1")
    ...
    .replace(/:\s*(string|number|boolean|void|never|any|unknown|object|symbol|bigint|null|undefined|Promise|Array|Record|Partial|Required|Readonly|Pick|Omit|Exclude|Extract|NonNullable|ReturnType|InstanceType|Parameters|ConstructorParameters)\b/g,
      (m, t) => `: ${C.type(t)}`)
    // Function calls
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-surface-raised border-b border-surface-border">
        ...
        <div className="flex items-center gap-2">
          <div className="hidden sm:flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-surface-card border border-surface-border">
            ...
            <span className="text-[10px] font-mono font-medium text-[var(--color-text-muted)]">{meta.label}</span>
          </div>
          <span className="text-[10px] text-[var(--color-text-muted)] hidden md:block tabular-nums">{lineCount} lines</span>
          <button
            onClick={handleCopy}
          ...
          >
            <AnimatePresence mode="wait">
              {copied ? (
                <motion.div key="check" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} transition={{ duration: 0.15 }} className="flex items-center gap-1">
                ...
                >
                  {showLineNumbers && (
                    <span
                      ...
                      style={{ color: "#30363d", fontSize: "11px", minWidth: "2.5em", lineHeight: "1.75" }}
                    >
                      {lineNum}
                    </span>
                  )}
